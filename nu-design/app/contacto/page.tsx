    ...
    <div className={`min-h-dvh flex flex-col justify-between transition-colors duration-1000 relative overflow-x-hidden py-4 md:py-6 ${
        ...
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`fixed inset-0 z-30 backdrop-blur-3xl flex flex-col justify-between p-8 pt-24 md:hidden ${
            ...
            <div className="flex flex-col space-y-6 text-xl font-medium tracking-wide">
              ...
              <Link href="/contacto" onClick={() => setIsMobileMenuOpen(false)} className="text-red-500 font-bold border-b border-zinc-500/20 pb-3 flex justify-between items-center"><span>Contacto</span><i className="fa-solid fa-arrow-right text-sm"></i></Link>
      ...
      <main className="w-full max-w-4xl mx-auto px-4 py-8 z-10 flex flex-col items-center">
        <div className="text-center mb-10 space-y-2">
          <div className="w-20 h-20 rounded-full overflow-hidden mx-auto border-2 border-red-500/50 mb-3 relative">
            <Image src="/garic-edume.jpg" alt="Garic Edume" fill className="object-cover" />
          </div>
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-light tracking-tight">Conectemos con <span className="font-semibold text-red-500">propósito</span></h1>
          <p className="text-[10px] md:text-xs font-light tracking-widest uppercase opacity-75">Garic Edume • Dirección Creativa & Diseño de Alta Gama</p>
        </div>

        <div className="w-full space-y-4">
          <a href="mailto:nubellstore@gmail.com" className={`block p-6 rounded-3xl border backdrop-blur-xl transition-all ${theme === 'dark' ? 'bg-zinc-900/40 border-white/15' : 'bg-white/60 border-black/10'}`}>
            <span className="text-[10px] uppercase font-bold text-red-500 tracking-wider">Correo Electrónico Oficial</span>
            <h3 className="text-lg font-bold mt-1 underline">nubellstore@gmail.com</h3>
          </a>
        </div>
      </main>
