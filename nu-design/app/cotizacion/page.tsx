...
export default function CotizacionPage() {
  ...
  <nav className="hidden md:flex items-center space-x-3 text-base font-medium">
        ...
        <main className="w-full max-w-4xl mx-auto px-6 py-12 z-10 flex flex-col items-center">
          ...
          {!isSubmitted ? (
            <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className={`w-full backdrop-blur-2xl border rounded-3xl p-8 md:p-12 shadow-2xl space-y-8 ${theme === 'dark' ? 'bg-zinc-900/50 border-white/15' : 'bg-white/70 border-zinc-300'}`}>
              ...
              {/* 8. VERIFICACIÓN ANTI-BOTS Y TÉRMINOS SIN FIRMA */}
        ...
        {/* MODAL DE TÉRMINOS Y CONDICIONES (TOTALMENTE LIMPIO, SIN DIBUJO DE FIRMA) */}
        <AnimatePresence>
          {showTermsModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
              <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className={`max-w-2xl w-full max-h-[80vh] overflow-y-auto p-8 rounded-3xl border shadow-2xl space-y-6 ${theme === 'dark' ? 'bg-zinc-950 border-white/20 text-zinc-200' : 'bg-white border-zinc-300 text-zinc-800'}`}>
                <div className="flex justify-between items-center border-b border-white/10 pb-4">
                  <h3 className="text-lg font-bold text-red-500 uppercase tracking-wide">TÉRMINOS Y CONDICIONES DEL SERVICIO • NU-DESIGN</h3>
                ...
                <div className="text-xs space-y-4 font-light leading-relaxed opacity-90">
                  ...
                  <p><strong>4. Pagos y Reembolsos:</strong> Los anticipos o pagos únicos no son reembolsables una vez iniciado el proceso de diseño activo.</p>
                </div>

                <div className="pt-4 flex justify-end">
                  <button type="button" onClick={() => { setAcceptedTerms(true); setShowTermsModal(false); }} className="px-8 py-3 bg-red-600 hover:bg-red-700 text-white rounded-full text-xs font-semibold uppercase tracking-wider cursor-pointer shadow-xl">
                    Entendido y Aceptar
                  </button>
                </div>
              </motion.div>
